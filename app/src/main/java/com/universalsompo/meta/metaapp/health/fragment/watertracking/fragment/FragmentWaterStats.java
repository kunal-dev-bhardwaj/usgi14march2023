...
public class FragmentWaterStats extends Fragment implements ResponseListener {
    private View v;
    ...
    private SwitchButton waterReminder;
    WaterReminderDatabaseHelper db;
    private RelativeLayout alarm_layout;
    FoodReminderDatabaseHelper fdb;


    @Nullable
    @Override
    public View onCreateView(final LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        v = inflater.inflate(R.layout.fragment_water_stats, container,false);
        ...
        fdb = new FoodReminderDatabaseHelper(getActivity());

        alarm_layout = v.findViewById(R.id.rl_fr);

        waterReminder = v.findViewById(R.id.water_reminder_btn);

        boolean water_reminder_available = db.CheckIsDataAlreadyInDBorNotID(pref.getUID(), "Water Reminder","interval");
        if (water_reminder_available==true){
            String water_status_remin = db.getStatusofReminder(pref.getUID(), "Water Reminder","interval");
            if (water_status_remin.equalsIgnoreCase("active")){
                int water_status_count = db.getStatusofReminder(pref.getUID());
                waterReminder.setChecked(true);
            }else {
                waterReminder.setChecked(false);
            }
        }  else {
            waterReminder.setChecked(false);
        ...
        alarm_layout.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (NetworkUtils.isConnected(getActivity())) {
                    FragmentsTransactionsUtils.replaceFragmentKeepPrevious(getActivity(), new FragmentWaterReminder(), R.id.main_frame1, FragmentsHealthTags.FRAGMENT_WATER_REMINDER);
                    binder.detect(FragmentsHealthTags.FRAGMENT_WATER_REMINDER);
                } else {
                    Toast.makeText(getActivity(), "You are not connected to internet", Toast.LENGTH_SHORT).show();
                }
            }
        });
        waterReminder.setOnCheckedChangeListener(new SwitchButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(SwitchButton view, boolean isChecked) {
                if (isChecked){
                    if (NetworkUtils.isConnected(getActivity())) {
                        long water_update = db.updateStatusofReminder(pref.getUID(), "Water reminder", "active");
                        FragmentsTransactionsUtils.replaceFragmentKeepPrevious(getActivity(), new FragmentWaterReminder(), R.id.main_frame1, FragmentsHealthTags.FRAGMENT_WATER_REMINDER);
                        binder.detect(FragmentsHealthTags.FRAGMENT_WATER_REMINDER);
                    } else {
                        Toast.makeText(getActivity(), "You are not connected to internet", Toast.LENGTH_SHORT).show();
                    ...
                    long update_lab_test = fdb.updateStatusofReminder(pref.getUID(), "Water reminder", "deactive");

                    long update_no_times = db.updateNoofTimeReminder(pref.getUID(),"Water Reminder","interval","","deactive");

                }
            }
        });
        yVals = new ArrayList<Entry>();
        callApi(RequestHealthConstants.GET_WATER_STATS_DATA);
        return v;
    }

    private void findViewById(View v) {
        mChart = v.findViewById(R.id.linechartwaterstats);
        tvfromdate = v.findViewById(R.id.tv_fromdatewater);
        tvtodate = v.findViewById(R.id.tv_todatewater);
    }

    private void callApi(Integer id) {
        if (id == RequestHealthConstants.GET_WATER_STATS_DATA) {
            ...
            ProjectVolleyRequest req = new ProjectVolleyRequest(getActivity(), object, UrlHealthConstants.GET_WATER_STATS_DATA, this, RequestHealthConstants.GET_WATER_STATS_DATA);
            req.execute();
        }
    }
    ...
    public void onSuccess(JSONObject object, int Tag) {
        if(Tag == RequestHealthConstants.GET_WATER_STATS_DATA) {
            if(object.optString("Message").equalsIgnoreCase("Success")) {
                try {
                    ...
                    for (int i = 0; i < jsonArray.length(); i++) {
                        ...
                        StringTokenizer st = new StringTokenizer(InLiter, " ");
    ...
    private void setData() {
        ...
        LineDataSet dataSet = new LineDataSet(yVals, "All data in litres");
