...
public class GetCurrent extends AsyncTask<String, String, HashMap<String, String>> {
    Context mContext;
    private MySharedPreference pref;
    private ProgressDialog mProgressDialog;
    String current_bp;

    public GetCurrent(Context context, ProgressDialog mProgressDialog) {
    ...
    protected HashMap<String, String> doInBackground(String... params) {
        ...
        if (!isCancelled()) {
            try {
                ...
                String strUrl ="https://core.allizhealthtest.in/PHR/api/BMI/GetCurrent";
...
//            }
            if(resultMap!=null && resultMap.size()>0){
                ...
                pref.setLifecurrentbpjsondata(current_bp);

                LabTestLifestyle lab_test = new LabTestLifestyle(mContext,mProgressDialog);
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
                    lab_test.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
                } else {
                    lab_test.execute();
                }
            }
